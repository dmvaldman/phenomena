...
| P5 | Covert content is used by downstream reasoning (swaps flip answers) | **Partial** — read-side established, causal swap weak | [3.3_twohop](3.3_twohop/) | Unspoken intermediates (spider, France…) rank top-10 in band on 90% of items, readable before the answer (70%) — but swapping them flips the downstream answer only 10% (best config). Clause-restricted swaps produce type-correct/entity-wrong outputs ("Neigh", "Chirp") — the second hop runs on swapped content but entity identity doesn't transport cleanly at 8B (paper needed Haiku-4.5-scale for 54%) |
